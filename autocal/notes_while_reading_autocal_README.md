Maybe `--robust-debug` should be the default, so we don't need to supply that all the time.

If no `--work-dataset` is provided, does the script create a default one?
It would be nice to not have to repeat that arg in the docs all the time.

If no `--sim` or `--simulation` is provided then any `--speedup` arg
should just be ignored. We shouldn't have to write that out.
...
The "## Troubleshooting quick tips" section was bogus.
