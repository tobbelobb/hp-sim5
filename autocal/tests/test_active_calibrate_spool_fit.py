...
def test_anchor_step_gate_uses_spool_search_objective_not_acceptance_total(monkeypatch):
    ...
    def fake_dataset_with_modeled_lengths(
        ...
        *,
        ...
        _ = prefer_zero_tension_angles
        out = dict(dataset)
        out["_spool_r"] = float(np.median(np.asarray(spool_params["radii_mm"], dtype=float)))
        return out
    ...
    class FakeCostFn:
        ...
        def evaluate_detailed(self, anchor_vec):
            ...
            return SimpleNamespace(
                ...
                noise_metrics={
                    "chi2_red": 200.0,
                    "chi2_red_tau_d_trimmed_direct": 200.0,
                    "n_obs_trimmed": 60.0,
                    "tau_mad_mm": 0.6,
                    "params": 6,
                    "sigma_model_mm": 1.0,
    ...
    assert isinstance(history, list) and history
    assert history[0].get("anchor_step_triggered") is False
    assert history[0].get("current_search_total_cost") == pytest.approx(10.0, abs=1e-9)
    assert history[0].get("current_total_cost") == pytest.approx(110.0, abs=1e-9)
...
def test_spool_step_objective_uses_ellipse_internal_cost(monkeypatch):
    base = np.array([30.0, 30.0, 30.0], dtype=float)
    observed = {}
    ...
    class FakeCostFn:
        ...
        def evaluate(self, anchor_vec):
            _ = anchor_vec
            return float((self._spool_r - 31.0) ** 2.0)

        def evaluate_detailed(self, anchor_vec):
            _ = anchor_vec
            risk = 100.0 if self._spool_r <= 31.5 else 0.0
            return SimpleNamespace(
                total_cost=float((self._spool_r - 31.0) ** 2.0),
                noise_metrics={
                    "chi2_red": risk,
                    "chi2_red_tau_d_trimmed_direct": risk,
                    "n_obs_trimmed": 60.0,
                    "tau_mad_mm": 0.6,
                    "params": 6,
                    "sigma_model_mm": 1.0,
    ...
    def fake_coordinate_descent_spool(x0, *, lo, hi, kinds, max_iters, objective):
        ...
        good = np.asarray([31.0], dtype=float)
        bad = np.asarray([33.0], dtype=float)
        observed["good"] = float(objective(good))
        observed["bad"] = float(objective(bad))
        chosen = good if observed["good"] < observed["bad"] else bad
    ...
    assert observed["good"] == pytest.approx(0.0, abs=1e-9)
    assert observed["bad"] == pytest.approx(4.0, abs=1e-9)
    history = fit_info.get("history")
    assert isinstance(history, list) and history
    assert history[0].get("fitted_data_cost") == pytest.approx(0.0, abs=1e-9)
    assert history[0].get("spool_data_cost_fixed_anchors") == pytest.approx(50.0, abs=1e-9)
