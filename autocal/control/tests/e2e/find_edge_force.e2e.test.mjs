...
async function main() {
  ...
  try {
    ...
    computeStallSpeedThresholdDegPerSec(thresholds.sigmaAct, intervalMs / 1000);
    ...
    const logTrial = (label, force, result) => {
      ...
      console.log(`; edge-force ${label}: test=${formatValue(force)} moved=${result?.moved ? 'yes' : 'no'} travel=${travel}deg stalled=${result?.stalled ? 'yes' : 'no'}`);
    };
    ...
    const edgeResult = await findEdgeForce(send, {
      forceStart: 0.2,
      capForceLimit,
      trialFn: runTrial,
    ...
    console.log(`Edge force: ${formatValue(edgeResult.forceEdge)} N`);
    console.log(`D_max: ${formatValue(edgeResult.dMax, 3)} deg`);
    console.log(JSON.stringify(edgeResult, null, 2));
    success = Number.isFinite(edgeResult.forceEdge);

  } catch (err) {
    console.error(`E2E find-edge-force failed: ${err?.message || err}`);
  } finally {
