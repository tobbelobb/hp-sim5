...
import { createTapRecord, resolveNavigationCursorZoomAnchor, resolveTouchNavigationCursorTapAction, shouldHideNavigationCursorOnClick } from "./navigation_cursor_interactions.js";
...
import { getMachineMotorDiagnostics, resetMachineMotorDiagnostics } from "./motor-diagnostics.js";
import { bakeCableSceneUsdaSource } from "../../src/js/usd/cable_scene_baker.js";
import { PerformanceMonitor } from "./performance-monitor.js";
import { DEFAULT_PRESET_KEY, DEFAULT_VIEW_SCALE } from "./appState.js";
import { AVAILABLE_USDAS, HP4_USDA_KEY, RRF_CONFIG_VARIANTS_BY_USDA_KEY, buildPublicProjectUrl } from "./machineCatalog.js";
import { PRESET_GCODE_MAP, describeSelectedPresetFile, getPresetActionLabel, resolvePresetCommand } from "./commandPresetResolver.js";
import { buildUploadPresetConfig } from "./uploadPipeline.js";
import { MAX_VIEW_SCALE, MIN_VIEW_SCALE, ZOOM_EPSILON, ZOOM_FACTOR } from "./viewController.js";
import { KLIPPER_UPLOAD_PIPELINE } from "./workerController.js";
...
function initHpSim() {
  ...
  function scheduleExternalCommandReconnect(reason = '') {
    ...
    if (attemptCount % EXTERNAL_WS_RECONNECT_LOG_EVERY === 0) {
      ...
      console.log(
        `hp-sim-3d: tried external G-code stream ${attemptCount} times${messageSuffix}. Continuing to retry every ${Math.round(delay)}ms`,
      );
    }
  ...
  function connectExternalCommandStream() {
    ...
    externalCommandSocket.addEventListener('open', () => {
      ...
      if (attemptsBeforeConnect > 1) {
        console.log(`hp-sim-3d: external G-code stream connected after ${attemptsBeforeConnect} attempts:`, externalWsUrl);
      } else {
  ...
  function handleCanvasClick(event) {
    ...
    if (!shouldHideNavigationCursorOnClick({ navigationCursorActive, clickDetail: event.detail })) {
      return;
    }
    setNavigationCursorActive(false);
    event.preventDefault();
  ...
  function handleCanvasWheel(event) {
    ...
    applyZoomAtScale(
      currentViewScale * multiplier,
      resolveNavigationCursorZoomAnchor({ x: event.clientX, y: event.clientY }, navigationCursorActive)
    );
  }
  ...
  function ensureKlipperMcuCommandPlayerWorker() {
    ...
    klipperMcuCommandPlayerWorker.onmessageerror = (event) => {
...
export function createHpSimApp({ document: ownerDocument = document, canvas = null, controlsRoot = null } = {}) {
  let started = false;
  ...
  return {
    ...
    start() {
      ...
      if (ownerDocument.readyState === 'loading') {
        ownerDocument.addEventListener('DOMContentLoaded', initHpSim, { once: true });
      } else {
        initHpSim();
      }
    },
