...
function addMirrorSpool(world, side) {
  ...
  world.addComponent(spool, new PositionComponent(x, -0.5, 0.0));
  world.addComponent(spool, new PrevFinalPosComponent(x, -0.5, 0.0));
  world.addComponent(spool, new VelocityComponent(0.0, 0.0, 0.0));
  ...
  world.addComponent(spool, new OrientationComponent());
  world.addComponent(spool, new PrevFinalOrientationComponent());
  world.addComponent(spool, new AngularVelocityComponent(0.0, 0.0, 0.0));
  world.addComponent(spool, new MomentOfInertiaComponent(spoolInertia));

  const anchor = world.createEntity();
  world.addComponent(anchor, new PositionComponent(x, 0.5, 0.0));
  world.addComponent(anchor, new VelocityComponent(0.0, 0.0, 0.0));
  ...
  world.addComponent(spool, new CableLinkComponent(spoolPos.x, spoolPos.y, spoolPos.z, null, PLANE_NORMAL));
  world.addComponent(anchor, new CableLinkComponent(anchorPos.x, anchorPos.y, anchorPos.z, null, PLANE_NORMAL));

  const tang = tangentFromPointToSphere(anchorPos, spoolPos, spoolRadius, PLANE_NORMAL, cw);
  const dist = tang.a_attach.clone().subtract(tang.a_sphere).length();
  const joint = world.createEntity();
  if (side === 'left') {
    world.addComponent(joint, new CableJointComponent(anchor, spool, dist, tang.a_attach, tang.a_sphere));
  } else {
    world.addComponent(joint, new CableJointComponent(spool, anchor, dist, tang.a_sphere, tang.a_attach));
  }
