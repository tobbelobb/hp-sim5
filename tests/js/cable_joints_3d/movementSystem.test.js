...
import { MovementSystem } from '../../../src/js/cable_joints_3d/commonSystems.js';

describe('MovementSystem (3D)', () => {
  test('updates position based on velocity and dt', () => {
    const world = new World();
    const e = world.createEntity();
    world.addComponent(e, new PositionComponent(1, 2, 3));
    world.addComponent(e, new VelocityComponent(3, 4, 5));
    const dt = 0.5;
    ...
    expect(pos.z).toBeCloseTo(3 + 5 * dt);
  });

  test('does not update entities missing components', () => {
    const world = new World();
    const e1 = world.createEntity();
    world.addComponent(e1, new PositionComponent(5, 5, 5));
    // missing VelocityComponent
    const e2 = world.createEntity();
    world.addComponent(e2, new VelocityComponent(1, 1, 1));
    const dt = 1.0;
    const system = new MovementSystem();
    system.update(world, dt);
