...
class PBDCableConstraintSolver:
    ...
    def update(self, world, _dt_unused):
        ...
        solver_iterations_resource = world.get_resource('cableSolverIterations')
        if solver_iterations_resource is not None and np.isfinite(solver_iterations_resource):
            iterations = int(max(1, min(16, int(np.floor(solver_iterations_resource)))))
        else:
            iterations = 2
