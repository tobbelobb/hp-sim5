...
{
	explicit SemaphoreControlBlock(SemaphoreKind kindIn) noexcept
	: kind(kindIn)
	{
...
std::chrono::milliseconds ToDuration(TickType_t ticks) noexcept
{
	...
	return std::chrono::milliseconds(static_cast<int64_t>(ticks * 1000ull / TicksPerSecond));
}

void RemoveTask(TaskControlBlock* tcb) noexcept
{
	std::lock_guard<std::mutex> lock(tasksMutex);
	auto it = std::find_if(allTasks.begin(), allTasks.end(), [tcb](const std::unique_ptr<TaskControlBlock>& ptr) noexcept
	{
		return ptr.get() == tcb;
	});
	if (it != allTasks.end())
...
void vTaskDelayUntil(TickType_t* const lastWakeTime, const TickType_t ticksToWait) noexcept
{
							   ...
							   StaticTask_t* taskBuffer) noexcept
{
	...
	{
		raw->taskBase->AttachHostHandle(static_cast<TaskHandle_t>(raw), raw->stackBase, stackDepth);
	}

	raw->nativeThread = std::thread([raw]()
	{
		currentTask = raw;
						   ...
						   TickType_t timeout) noexcept
{
	...
	if (!tcb->notified)
	{
		...
		else if (!tcb->notifyCv.wait_for(lock, duration, [tcb]() noexcept { return tcb->notified; }))
		{
			return pdFAIL;
		}
	}
...
BaseType_t xQueueSendFromISR(QueueHandle_t handle,
							 const void* item,
							 BaseType_t* higherPriorityTaskWoken) noexcept
{
	if (higherPriorityTaskWoken != nullptr)
...
{
	return static_cast<SemaphoreHandle_t>(new SemaphoreControlBlock(SemaphoreKind::Mutex));
}

SemaphoreHandle_t xSemaphoreCreateBinary() noexcept
{
	return static_cast<SemaphoreHandle_t>(new SemaphoreControlBlock(SemaphoreKind::Binary));
}
