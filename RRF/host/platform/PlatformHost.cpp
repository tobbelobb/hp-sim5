...
static bool CombineRrfPath(String<MaxFilenameLength>& result, const char* base,
...
bool Platform::Delete(const char* _ecv_array folder,
                      const char* _ecv_array filename) const noexcept
{
    String<MaxFilenameLength> location;
    return MassStorage::CombineName(location.GetRef(), folder, filename) &&
           MassStorage::Delete(location.GetRef(), ErrorMessageMode::messageUnlessMissing);
}
...
bool Platform::SysFileExists(const char* filename) const noexcept
{
    ...
    return MakeSysFileName(fullPath.GetRef(), filename) &&
           MassStorage::FileExists(fullPath.c_str());
#else
    return false;
#endif
}
...
FileStore* Platform::OpenSysFile(const char* filename, OpenMode mode) const noexcept
{
    ...
    String<MaxFilenameLength> fullPath;
    return MakeSysFileName(fullPath.GetRef(), filename)
               ? MassStorage::OpenFile(fullPath.c_str(), mode, 0)
               : nullptr;
#else
    return nullptr;
#endif
...
bool Platform::MakeSysFileName(const StringRef& result,
...
FileStore* Platform::OpenFile(const char* directory, const char* filename, OpenMode mode,
                              unsigned int preAllocSize) const noexcept
{
#if HAS_MASS_STORAGE
    String<MaxFilenameLength> fullPath;
...
#if HAS_MASS_STORAGE
    String<MaxFilenameLength> fullPath;
    return MassStorage::CombineName(fullPath.GetRef(), folder, filename) &&
           MassStorage::FileExists(fullPath.c_str());
#else
    return false;
#endif
...
{
    return (IsLoggingActive() && !logFileRrfPath.IsEmpty()) ? logFileRrfPath.c_str()
                                                            : nullptr;
}
...
void Platform::WriteLogEntryUnlocked(uint32_t messageLogLevel,
                                     const char* message) noexcept
{
    ...
    else
    {
        ...
        {
            prefix.printf("%04d-%02d-%02d %02d:%02d:%02d ", timeInfo.tm_year + 1900,
    ...
    const char* levelName =
        (messageLogLevel < 4u) ? MessageLogLevelNames[messageLogLevel] : "off";
...
GCodeResult Platform::StartLogging(const char* filename, const StringRef& reply) noexcept
{
    const char* requested =
        (filename != nullptr && filename[0] != 0) ? filename : DEFAULT_LOG_FILE;
...
GCodeResult Platform::ConfigureLogging(GCodeBuffer& gb, const StringRef& reply) noexcept
{
    if (gb.Seen('S'))
    {
        const auto newLevelRaw = static_cast<LogLevel::RawType>(
            gb.GetLimitedUIValue('S', LogLevel::off, LogLevel::NumValues));
    ...
    else
    {
        ...
        {
            reply.printf("Event logging is enabled at log level %s",
                         logLevelSetting.ToString());
        }
    }
...
ConfigurableFolder::ConfigurableFolder(Platform& owner, ReadWriteLock& lock,
    ...
    : owner(owner), lockRef(lock), storageRef(storage), defaultValue(defaultValue)
{
}
...
bool ConfigurableFolder::EnsureTrailingSlash(
    String<MaxFilenameLength>& path) const noexcept
{
    if (!path.EndsWith('/'))
    {
        if (path.cat('/'))
                           ...
                           const char* fragment) noexcept
{
    ...
    const bool isRelative =
        frag[0] != '/' &&
        (std::strlen(frag) < 2 || !std::isdigit(static_cast<unsigned char>(frag[0])) ||
         frag[1] != ':');
...
GCodeResult ConfigurableFolder::Configure(const char* dir,
...
Platform::GetObjectModelArrayEntry(unsigned int) const noexcept
{
    return nullptr;
}
