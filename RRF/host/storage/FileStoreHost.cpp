...
: handle(), usageMode(FileUseMode::free), openCount(0), closeRequested(false), calcCrc(false), writeBuffer()
{
}
...
bool FileStore::Open(const char* filePath, OpenMode mode, uint32_t /*preAllocSize*/) noexcept
{
	...
	{
		newHandle->stream.seekp(0, (mode == OpenMode::append) ? std::ios::end : std::ios::beg);
