...
constexpr const char* MonthNames[] = {"Jan", "Feb", "Mar", "Apr", "May", "Jun",
                                      "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"};

bool ValidateVolumePrefix(std::string& path) noexcept
{
    if (path.size() >= 3 && std::isdigit(static_cast<unsigned char>(path[0])) &&
...
bool MassStorage::CombineName(const StringRef& outbuf, const char* directory,
...
FileStore* MassStorage::OpenFile(const char* filePath, OpenMode mode,
...
bool MassStorage::Delete(const StringRef& filePath, ErrorMessageMode errorMessageMode,
...
GCodeResult MassStorage::GetFileInfo(const char* filePath, GCodeFileInfo& info,
                                     bool quitEarly, GlobalVariables* customVars) noexcept
{
    (void)customVars;
    info.Init();
...
GCodeResult MassStorage::Mount(size_t, const StringRef& reply,
                               bool reportSuccess) noexcept
{
...
bool MassStorage::Rename(const char* oldFilePath, const char* newFilePath,
                         bool deleteExisting, bool messageIfFailed) noexcept
{
    (void)messageIfFailed;
    fs::path oldPath;
    fs::path newPath;
...
bool MassStorage::SetLastModifiedTime(const char* file, time_t t) noexcept
{
    ...
    const auto ftime =
        fs::file_time_type::clock::now() + (tp - std::chrono::system_clock::now());
...
void MassStorage::RecordSimulationTime(const char* printingFilePath,
                                       uint32_t simSeconds) noexcept
{
    fs::path hostPath;
...
MassStorage::InfoResult MassStorage::GetCardInfo(
    size_t, SdCardReturnedInfo& returnedInfo) noexcept
{
    std::error_code ec;
...
GCodeResult MassStorage::ConfigureSdCard(GCodeBuffer& gb,
                                         const StringRef& reply) noexcept(false)
{
    (void)gb;
