...
// Host-only stub for FansManager. All fan control requests are satisfied locally without
// hardware access.
class FansManager
{
public:
    ...
    bool CheckFans(bool) noexcept
    {
        return false;
    }

    GCodeResult ConfigureFanPort(GCodeBuffer& gb, const StringRef& reply)
        THROWS(GCodeException)
    {
        ...
        {
            state.pulsesPerRev =
                gb.GetLimitedFValue('K', MinFanPulsesPerRev, MaxFanPulsesPerRev);
            state.configured = true;
            seenParameter = true;
        }
        ...
        if (!seenParameter)
        {
            ...
            {
                reply.printf("Fan %u: port %s, freq %.1f Hz, PPR %.1f", (unsigned)fanNum,
                             state.portName.IsEmpty() ? "unset" : state.portName.c_str(),
                             (double)state.frequency, (double)state.pulsesPerRev);
            }
        }
    ...
    bool ConfigureFan(unsigned int mcode, size_t fanNum, GCodeBuffer& gb,
                      const StringRef& reply, bool& error) THROWS(GCodeException)
    {
        ...
        if (mcode == 106)
        {
            if (gb.Seen('T'))
            {
                float temps[2] = {state.triggerTemperatures[0],
                                  state.triggerTemperatures[1]};
                size_t count = 2;
        ...
        reply.printf(
            "Fan %u%s, speed %d%%, min %d%%, max %d%%", static_cast<unsigned>(fanNum),
            state.name.IsEmpty() ? "" : state.name.c_str(), (int)(state.speed * 100.0f),
            (int)(state.minSpeed * 100.0f), (int)(state.maxSpeed * 100.0f));
        if (state.thermostatic)
        {
            reply.catf(", thermostatic %.1f:%.1fC sensors:",
    ...
    {
        return (IsValidFan(fanNum) && states[fanNum].configured) ? states[fanNum].speed
                                                                 : 0.0f;
    }
...
#if SUPPORT_REMOTE_COMMANDS
    GCodeResult ConfigureFanPort(const CanMessageGeneric&,
    ...
    GCodeResult ConfigureFan(const CanMessageFanParameters&,
                             const StringRef& reply) noexcept
    {
        reply.copy(UnsupportedMessage);
        return GCodeResult::warningNotSupported;
    ...
    unsigned int PopulateFansReport(CanMessageFansReport&) noexcept
    {
        return 0;
    }
#endif
    ...
    bool WriteFanSettings(FileStore*) const noexcept
    {
        return false;
    }
#endif
...
private:
    struct FanState
    {
        ...
        float minSpeed{0.0f};
        float maxSpeed{1.0f};
        float triggerTemperatures[2]{DefaultHotEndFanTemperature,
                                     DefaultHotEndFanTemperature};
    ...
    static constexpr const char* _ecv_array UnsupportedMessage =
        "Fan control not implemented on host build";
