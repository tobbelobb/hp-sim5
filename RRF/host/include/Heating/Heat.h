...
public:
    ...
    GCodeResult TuneHeater(GCodeBuffer& gb, const StringRef& reply)
        THROWS(GCodeException);
    GCodeResult ConfigureSensor(GCodeBuffer& gb, const StringRef& reply)
        THROWS(GCodeException);
    GCodeResult SetPidParameters(unsigned int heater, GCodeBuffer& gb,
                                 const StringRef& reply) THROWS(GCodeException);
    GCodeResult HandleM143(GCodeBuffer& gb, const StringRef& reply)
        THROWS(GCodeException);

    ReadLockedPointer<TemperatureSensor> FindSensor(int sensorNumber) const noexcept;
    ReadLockedPointer<TemperatureSensor> FindSensorAtOrAbove(
        unsigned int sensorNumber) const noexcept;
    ...
    bool HeaterAtSetTemperature(int heater, bool waitWhenCooling, float tolerance,
                                bool waitOnFault) const noexcept;

    GCodeResult ConfigureHeater(GCodeBuffer& gb, const StringRef& reply)
        THROWS(GCodeException);
    GCodeResult ConfigureHeaterMonitoring(size_t heater, GCodeBuffer& gb,
                                          const StringRef& reply) THROWS(GCodeException);

    void SetActiveTemperature(int heater, float t) THROWS(GCodeException)
    {
        SetTemperature(heater, t, true);
    }
    void SetStandbyTemperature(int heater, float t) THROWS(GCodeException)
    {
        SetTemperature(heater, t, false);
    }
    void SetTemperature(int heater, float t, bool activeNotStandby)
        THROWS(GCodeException);

    GCodeResult SetActiveOrStandby(int heater, const Tool* _ecv_null tool, bool active,
    ...
    void SetExtrusionFeedForward(unsigned int heater, float pwmBoost,
                                 float tempBoost) const noexcept;
    ...
    void ProcessRemoteSensorsReport(CanAddress src,
                                    const CanMessageSensorTemperatures& msg) noexcept;
    void ProcessRemoteHeatersReport(CanAddress src,
                                    const CanMessageHeatersStatus& msg) noexcept;
    void ProcessRemoteHeaterTuningReport(
        CanAddress src, const CanMessageHeaterTuningReport& msg) noexcept;
#endif

#if SUPPORT_REMOTE_COMMANDS
    GCodeResult ConfigureHeater(const CanMessageGeneric& msg,
                                const StringRef& reply) noexcept;
    GCodeResult ProcessM307V1(const CanMessageHeaterModelV2& msg,
                              const StringRef& reply) noexcept;
    GCodeResult ProcessM308(const CanMessageGeneric& msg,
                            const StringRef& reply) noexcept;
    GCodeResult SetFaultDetection(const CanMessageSetHeaterFaultDetectionParameters& msg,
                                  const StringRef& reply) noexcept;
    GCodeResult SetHeaterMonitors(const CanMessageSetHeaterMonitors& msg,
                                  const StringRef& reply) noexcept;
    GCodeResult SetTemperature(const CanMessageSetHeaterTemperature& msg,
                               const StringRef& reply) noexcept;
    GCodeResult TuningCommand(const CanMessageHeaterTuningCommand& msg,
                              const StringRef& reply) noexcept;
    GCodeResult ApplyFeedForward(const CanMessageHeaterFeedForwardV1& msg,
                                 const StringRef& reply) noexcept;
#endif

    static TaskHandle GetHeatTask() noexcept
    {
        return heatTaskHandle;
    }
...
private:
    ...
    SensorState& GetSensorState(size_t sensor) noexcept
    {
        return sensorStates[sensor];
    }
    const SensorState& GetSensorState(size_t sensor) const noexcept
    {
        return sensorStates[sensor];
    }
    HeaterConfig& GetHeaterConfig(size_t heater) noexcept
    {
        return heaterConfigs[heater];
    }
    const HeaterConfig& GetHeaterConfig(size_t heater) const noexcept
    {
        return heaterConfigs[heater];
    }
...
template <size_t N>
inline bool Heat::HeaterMatchesList(int heater,
                                    const std::array<int8_t, N>& list) const noexcept
{
...
inline void Heat::SetChamberHeater(size_t index, int heater) noexcept
{
    ...
    {
        chamberHeaters[index] =
            (IsValidHeater(heater)) ? static_cast<int8_t>(heater) : -1;
    }
}
...
inline GCodeResult Heat::SetOrReportHeaterModel(GCodeBuffer& gb, const StringRef& reply)
    THROWS(GCodeException)
{
    ...
    {
        reply.printf("Heater %u model: %s, S=%.2f, A=%.2f, C=%.2f, D=%.2f, V=%.1f",
                     heater, config.usePid ? "PID" : "bang-bang", (double)config.pwmLimit,
                     (double)config.modelGain, (double)config.timeConstant,
                     (double)config.deadTime, (double)config.supplyVoltage);
    }
...
inline GCodeResult Heat::ConfigureSensor(GCodeBuffer& gb, const StringRef& reply)
    THROWS(GCodeException)
{
    gb.MustSee('S');
    ...
    if (!anyParameter)
    {
        ...
        {
            reply.printf("Sensor %u type %s reading %.1fC", sensorNum,
                         (sensor.typeName.IsEmpty()) ? "unknown"
                                                     : sensor.typeName.c_str(),
...
inline GCodeResult Heat::SetPidParameters(unsigned int, GCodeBuffer&, const StringRef&)
    THROWS(GCodeException)
{
    return GCodeResult::warningNotSupported;
}

inline GCodeResult Heat::HandleM143(GCodeBuffer& gb, const StringRef& reply)
    THROWS(GCodeException)
{
    const size_t heaterNumber =
        (gb.Seen('H')) ? gb.GetLimitedUIValue('H', MaxHeaters) : 1;
    ...
    if (!seenParameter)
    {
        reply.printf("Heater %zu limits %.1f to %.1fC", heaterNumber,
...
inline ReadLockedPointer<TemperatureSensor> Heat::FindSensorAtOrAbove(
    unsigned int) const noexcept
{
    return ReadLockedPointer<TemperatureSensor>(nullptr, nullptr);
}

inline float Heat::GetSensorTemperature(int sensorNum,
                                        TemperatureError& err) const noexcept
{
...
inline bool Heat::HeaterAtSetTemperature(int heater, bool, float tolerance,
                                         bool) const noexcept
{
    if (!IsValidHeater(heater))
    {
...
inline GCodeResult Heat::ConfigureHeater(GCodeBuffer& gb, const StringRef& reply)
    THROWS(GCodeException)
{
    ...
    if (!seenParameter)
    {
        ...
        {
            reply.printf("Heater %zu: port %s, sensor %d, freq %.1f Hz", heater,
                         config.portName.IsEmpty() ? "unset" : config.portName.c_str(),
                         config.sensorNumber, (double)config.pwmFrequency);
        }
    }
...
inline GCodeResult Heat::ConfigureHeaterMonitoring(size_t heater, GCodeBuffer& gb,
    ...
    THROWS(GCodeException)
{
    ...
    if (!seenParameter)
    {
        reply.printf("Heater %zu fault timeout %.1f s", heater,
                     (double)config.faultTimeout);
    }
    else
    {
        config.created = true;
...
inline void Heat::SetTemperature(int heater, float t, bool activeNotStandby)
    THROWS(GCodeException)
{
    if (!IsValidHeater(heater))
    {
    ...
    if (config.sensorNumber >= 0 &&
        static_cast<size_t>(config.sensorNumber) < sensorStates.size())
    {
...
inline GCodeResult Heat::SetActiveOrStandby(int heater, const Tool* _ecv_null tool,
                                            bool active, const StringRef&) noexcept
{
    if (!IsValidHeater(heater))
    {
        return GCodeResult::error;
...
#if SUPPORT_CAN_EXPANSION
inline void Heat::ProcessRemoteSensorsReport(CanAddress,
                                             const CanMessageSensorTemperatures&) noexcept
{
}

inline void Heat::ProcessRemoteHeatersReport(CanAddress,
                                             const CanMessageHeatersStatus&) noexcept
{
}

inline void Heat::ProcessRemoteHeaterTuningReport(
    CanAddress, const CanMessageHeaterTuningReport&) noexcept
{
}
#endif

#if SUPPORT_REMOTE_COMMANDS
inline GCodeResult Heat::ConfigureHeater(const CanMessageGeneric&,
                                         const StringRef&) noexcept
{
    return GCodeResult::warningNotSupported;
}

inline GCodeResult Heat::ProcessM307V1(const CanMessageHeaterModelV2&,
                                       const StringRef&) noexcept
{
    return GCodeResult::warningNotSupported;
}
...
inline GCodeResult Heat::SetFaultDetection(
    const CanMessageSetHeaterFaultDetectionParameters&, const StringRef&) noexcept
{
    return GCodeResult::warningNotSupported;
}

inline GCodeResult Heat::SetHeaterMonitors(const CanMessageSetHeaterMonitors&,
                                           const StringRef&) noexcept
{
    return GCodeResult::warningNotSupported;
}

inline GCodeResult Heat::SetTemperature(const CanMessageSetHeaterTemperature&,
                                        const StringRef&) noexcept
{
    return GCodeResult::warningNotSupported;
}

inline GCodeResult Heat::TuningCommand(const CanMessageHeaterTuningCommand&,
                                       const StringRef&) noexcept
{
    return GCodeResult::warningNotSupported;
}

inline GCodeResult Heat::ApplyFeedForward(const CanMessageHeaterFeedForwardV1&,
                                          const StringRef&) noexcept
{
    return GCodeResult::warningNotSupported;
}
