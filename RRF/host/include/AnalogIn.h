...
inline bool EnableChannel(AdcInput, AnalogInCallbackFunction, CallbackParameter,
                          uint32_t) noexcept
{
    return false;
}
inline bool SetCallback(AdcInput, AnalogInCallbackFunction, CallbackParameter,
                        uint32_t) noexcept
{
    return false;
}
inline bool IsChannelEnabled(AdcInput) noexcept
{
    return false;
}
...
inline uint16_t ReadChannel(AdcInput) noexcept
{
    return 0;
}
...
inline bool EnableTemperatureSensor(unsigned int, AnalogInCallbackFunction,
                                    CallbackParameter, uint32_t, unsigned int) noexcept
{
    return false;
}
inline void EnableTemperatureSensor(AnalogInCallbackFunction, CallbackParameter,
                                    uint32_t) noexcept
{
}
[[noreturn]] inline void TaskLoop(void*) noexcept
{
    std::abort();
}
using AdcTaskHookFunction = void() noexcept;
inline AdcTaskHookFunction* SetTaskHook(AdcTaskHookFunction* fn) noexcept
{
    return fn;
}
}  // namespace AnalogIn
...
inline uint16_t AnalogInReadChannel(AnalogChannelNumber) noexcept
{
    return 0;
}
using AnalogCallback_t = void (*)() noexcept;
inline AnalogCallback_t AnalogInSetCallback(AnalogCallback_t fn) noexcept
{
    return fn;
}
...
inline bool AnalogInCheckReady(uint32_t = 0xFFFFFFFFu) noexcept
{
    return true;
}
inline AnalogChannelNumber PinToAdcChannel(uint32_t) noexcept
{
    return 0;
}
inline AnalogChannelNumber GetTemperatureAdcChannel() noexcept
{
    return 0;
}
}  // namespace LegacyAnalogIn
