...
public:
    ...
    GCodeResult EnableInterface(unsigned int interface, int mode, const StringRef& ssid,
                                const StringRef& reply) noexcept;
    GCodeResult EnableProtocol(unsigned int interface, NetworkProtocol protocol, int port,
                               uint32_t ip, int secure, const StringRef& reply) noexcept;
    GCodeResult DisableProtocol(unsigned int interface, NetworkProtocol protocol,
                                const StringRef& reply) noexcept;
    GCodeResult ReportProtocols(unsigned int interface,
                                const StringRef& reply) const noexcept;

    GCodeResult HandleWiFiCode(int mcode, GCodeBuffer& gb, const StringRef& reply,
                               OutputBuffer*& longReply) noexcept;
    ...
    GCodeResult ConfigureNetworkProtocol(GCodeBuffer& gb,
                                         const StringRef& reply) noexcept;
    GCodeResult GetNetworkState(unsigned int interface, const StringRef& reply) noexcept;
    int EnableState(unsigned int) const noexcept
    {
        return 0;
    }

    IPAddress GetIPAddress(unsigned int) const noexcept
    {
        return ipAddress;
    }
    void SetReportedIPAddress(IPAddress ip) noexcept
    {
        reportedIpAddress = ip;
    }
    void SetEthernetIPAddress(IPAddress ip, IPAddress nm, IPAddress gw) noexcept;
    IPAddress GetNetmask(unsigned int) const noexcept
    {
        return netMask;
    }
    IPAddress GetGateway(unsigned int) const noexcept
    {
        return gateway;
    }
    bool UsingDhcp(unsigned int) const noexcept
    {
        return usingDhcp;
    }
    GCodeResult SetMacAddress(unsigned int, const MacAddress& mac,
                              const StringRef& reply) noexcept;
    const MacAddress& GetMacAddress(unsigned int) const noexcept
    {
        return macAddress;
    }
    const char* _ecv_array GetHostname() const noexcept
    {
        return hostname;
    }
    ...
    bool FindResponder(Socket*, NetworkProtocol) noexcept
    {
        return false;
    }

    bool StartClient(NetworkInterface*, NetworkProtocol) noexcept
    {
        return false;
    }
...
private:
    static constexpr const char* _ecv_array UnsupportedMessage =
        "Networking not implemented on host build";
...
inline GCodeResult Network::EnableInterface(unsigned int, int, const StringRef&,
                                            const StringRef& reply) noexcept
{
    reply.copy(UnsupportedMessage);
...
inline GCodeResult Network::EnableProtocol(unsigned int, NetworkProtocol, int, uint32_t,
                                           int, const StringRef& reply) noexcept
{
    reply.copy(UnsupportedMessage);
    return GCodeResult::warningNotSupported;
}

inline GCodeResult Network::DisableProtocol(unsigned int, NetworkProtocol,
                                            const StringRef& reply) noexcept
{
    reply.copy(UnsupportedMessage);
    return GCodeResult::warningNotSupported;
...
inline GCodeResult Network::ReportProtocols(unsigned int,
                                            const StringRef& reply) const noexcept
{
...
inline GCodeResult Network::HandleWiFiCode(int, GCodeBuffer&, const StringRef& reply,
...
inline GCodeResult Network::ConfigureNetworkProtocol(GCodeBuffer&,
                                                     const StringRef& reply) noexcept
{
    reply.copy(UnsupportedMessage);
    return GCodeResult::warningNotSupported;
...
inline void Network::SetEthernetIPAddress(IPAddress ip, IPAddress nm,
...
inline GCodeResult Network::SetMacAddress(unsigned int, const MacAddress& mac,
                                          const StringRef& reply) noexcept
{
    macAddress = mac;
