...
inline StandardDriverStatus GetStatus(size_t, bool, bool) noexcept
{
    return StandardDriverStatus();
}

inline float GetTmcDriversTemperature(unsigned int) noexcept
{
    return 0.0f;
}

inline bool EnablePhaseStepping(size_t, bool) noexcept
{
    return false;
}
inline bool IsPhaseSteppingEnabled(size_t) noexcept
{
    return false;
}
inline uint16_t GetMicrostepPosition(size_t) noexcept
{
    return 0;
}
inline int GetMicrostepShift(size_t) noexcept
{
    return 0;
}

inline const char* CheckStallDetectionEnabled(size_t, float) noexcept
{
    return nullptr;
}
