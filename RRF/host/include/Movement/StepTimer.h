...
public:
    ...
    static void ProcessTimeSyncMessage(const CanMessageTimeSync&, size_t,
                                       uint16_t) noexcept
    {
    }
    static uint32_t ConvertToMasterTime(uint32_t localTime) noexcept
    {
        return localTime;
    }
    static uint32_t GetMasterTime() noexcept
    {
        return NowTicks();
    }
    static bool CheckSynced() noexcept
    {
        return true;
    }
