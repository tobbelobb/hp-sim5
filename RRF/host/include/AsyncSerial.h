...
public:
	...
	AsyncSerial(void* = nullptr, int = 0, uint32_t = 0, size_t = 0, size_t = 0, OnBeginFn = nullptr, OnEndFn = nullptr) noexcept {}

	void begin(uint32_t, UARTModes = Mode_8N1) noexcept {}
	void end() noexcept {}
	int available() noexcept override { return 0; }
	int read() noexcept override { return -1; }
	void flush() noexcept override {}
	size_t write(uint8_t) noexcept override { return 1; }
	size_t write(const uint8_t*, size_t len) noexcept override { return len; }
	size_t canWrite() noexcept override { return 0; }
	...
	uint32_t getInterruptPriority() noexcept { return 0; }

	InterruptCallbackFn SetInterruptCallback(InterruptCallbackFn f) noexcept { return f; }
	OnTransmissionEndedFn SetOnTxEndedCallback(OnTransmissionEndedFn fn, CallbackParameter) noexcept { return fn; }

	Errors GetAndClearErrors() noexcept { return Errors{}; }
