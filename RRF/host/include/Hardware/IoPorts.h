...
public:
    ...
    bool SetMode(PinAccess) noexcept
    {
        return true;
    }
    ...
    static size_t AssignPorts(GCodeBuffer&, const StringRef&, PinUsedBy neededFor,
                              size_t numPorts, IoPort* _ecv_from const ports[],
                              const PinAccess access[]) noexcept
    {
        ...
        for (; assigned < numPorts; ++assigned)
        {
            ...
            {
                ports[assigned]->Allocate(nullptr, dummyReply, neededFor,
                                          access[assigned]);
            }
        }
        return numPorts;
    ...
    bool AssignPort(GCodeBuffer&, const StringRef&, PinUsedBy neededFor,
    ...
    static size_t AssignPorts(const char* _ecv_array pinNames, const StringRef& reply,
                              PinUsedBy neededFor, size_t numPorts,
                              IoPort* _ecv_from const ports[],
                              const PinAccess access[]) noexcept
    {
        size_t assigned = 0;
        ...
        while (assigned < numPorts)
        {
            ...
            if (ports[assigned] != nullptr)
            {
                ports[assigned]->Allocate((name.IsEmpty()) ? nullptr : name.c_str(),
                                          reply, neededFor, access[assigned]);
            }
            ++assigned;
        }
        return numPorts;
    ...
    bool AssignPort(const char* _ecv_array pinName, const StringRef& reply,
                    PinUsedBy neededFor, PinAccess access) noexcept
    {
    ...
    {
        return logicalPin != NoLogicalPin;
    }
    ...
    bool IsHardwareInverted() const noexcept
    {
        return hardwareInvert;
    }
    bool GetTotalInvert() const noexcept
    {
        return totalInvert;
    }
    ...
    {
        return totalInvert ? !state : state;
    }

    bool AttachInterrupt(StandardCallbackFunction, InterruptMode,
                         CallbackParameter) const noexcept
    {
        return false;
    }
...
#if SAME5x
    bool SetAnalogCallback(AnalogInCallbackFunction, CallbackParameter, uint32_t) noexcept
    {
        return false;
    }
    ...
    {
        return static_cast<uint16_t>(analogValue * 65535.0f);
    }
    ...
    void FastDigitalWriteLow() const noexcept
    {
        WriteDigital(false);
    }
    void FastDigitalWriteHigh() const noexcept
    {
        WriteDigital(true);
    }
    ...
    static void AppendPinNames(const StringRef& str, size_t numPorts,
                               const IoPort* const ports[]) noexcept
    {
    ...
    static CanAddress RemoveBoardAddress(const StringRef&) noexcept
    {
        return 0;
    }
#else
    static bool RemoveBoardAddress(const StringRef&) noexcept
    {
        return true;
    }
#endif
    ...
    static bool ReadPin(Pin) noexcept
    {
        return false;
    }
...
protected:
    bool Allocate(const char* _ecv_array pinName, const StringRef&, PinUsedBy neededFor,
                  PinAccess access) noexcept
    {
        (void)neededFor;
        (void)access;
        ...
        // Treat every assignment as successful and give the port a deterministic but
        // unique logical pin.
    ...
    static const char* _ecv_array TranslatePinAccess(PinAccess) noexcept
    {
        return "virtual";
    }
...
public:
    ...
    PwmFrequency GetFrequency() const noexcept
    {
        return frequency;
    }
