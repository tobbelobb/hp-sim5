...
{
public:
    ConfigurableFolder(Platform& owner, ReadWriteLock& lock,
...
public:
    ...
    GCodeResult DiagnosticTest(GCodeBuffer& gb, const StringRef& reply,
                               OutputBuffer* _ecv_null& buf, unsigned int d)
    {
        return GCodeResult::ok;
    };
    static constexpr unsigned int NumPlatformDiagnosticParts = 7;
    static bool SetDebugBufferSize(unsigned int) noexcept
    {
        return true;
    }
    ...
    static bool IsrDebugPutc(char) noexcept
    {
        return true;
    }
    ...
    bool IsDateTimeSet() const noexcept
    {
        return true;
    }
    ...
    void AppendAuxReply(size_t auxNumber, const char* _ecv_array msg,
                        bool rawMessage) noexcept {};
    ...
    ReadLockedPointer<ZProbe> GetZProbeOrDefault(size_t probeNumber) noexcept
    {
        return endstops.GetZProbeOrDefault(probeNumber);
    }

    // --- Filesystem Abstraction ---
    bool SysFileExists(const char* filename) const noexcept;
#if HAS_MASS_STORAGE || HAS_SBC_INTERFACE
    bool Delete(const char* _ecv_array folder,
                const char* _ecv_array filename) const noexcept;
    ...
    FileStore* OpenFile(const char* directory, const char* filename, OpenMode mode,
                        uint32_t preAllocSize = 0) const noexcept;
    ...
    GetMacroDir() noexcept;  // Where the user-defined macros are
                             //
    ReadLockedPointer<const char> GetSysDir() const noexcept
    {
        return sysFolder.GetLockedPointer();
    }
    void AppendSysDir(const StringRef& path) const noexcept
    {
        sysFolder.AppendToString(path);
    }
    void SetSysDir(const char* path) noexcept;
    GCodeResult SetSysDir(const char* _ecv_array dir, const StringRef& reply) noexcept
    {
        return sysFolder.Configure(dir, reply);
    }
    GCodeResult SetWebDir(const char* _ecv_array dir, const StringRef& reply) noexcept
    {
        return webFolder.Configure(dir, reply);
    }
    ReadLockedPointer<const char> GetWebDir() const noexcept
    {
        return webFolder.GetLockedPointer();
    }
    void AppendWebDir(const StringRef& path) const noexcept
    {
        webFolder.AppendToString(path);
    }
    ...
    {
        return true;
    }  // Pretend PSU is always on
    bool IsAtxPowerControlled() const noexcept
    {
        return false;
    }
    bool IsDeferredPowerDown() const noexcept
    {
        return false;
    }
    GCodeResult HandleM80(GCodeBuffer& gb, const StringRef& reply)
    {
        return GCodeResult::ok;
    };
    GCodeResult HandleM81(GCodeBuffer& gb, const StringRef& reply)
    {
        return GCodeResult::ok;
    };
    GCodeResult HandleM575(GCodeBuffer& gb, const StringRef& reply)
    {
        return GCodeResult::ok;
    };
    GCodeResult SendI2cOrModbus(GCodeBuffer& gb, const StringRef& reply)
    {
        return GCodeResult::ok;
    };  // Handle M260
    GCodeResult ReceiveI2cOrModbus(GCodeBuffer& gb, const StringRef& reply)
    {
        return GCodeResult::ok;
    };  // Handle M261
        //
    ...
    //
    BoardType GetBoardType() const noexcept
    {
    ...
    {
        return 24.0f;
    }  // Prevent low-voltage warnings
#endif
    GCodeResult SetBuzzerPort(GCodeBuffer& gb, const StringRef& reply)
    {
        return GCodeResult::ok;
    }
    bool Beep(unsigned int freq, unsigned int ms) noexcept;  // Do nothing for beeps
    bool IsChanEnabled(size_t chan) const noexcept
    {
        return false;
    }
    bool IsChanRaw(size_t chan) const noexcept
    {
        return false;
    }
    ...
    const UniqueId& GetUniqueId() const noexcept
    {
        return uniqueId;
    }
    //
...
#endif
    //
    bool IsOutputOnExtrudeActive() const noexcept
    {
        return true;
    }
    ...
    Spindle& AccessSpindle(size_t slot) noexcept
    {
        return spindles[slot];
    }
...
protected:
    ...
    //	const ObjectModelClassDescriptor *_ecv_null GetObjectModelClassDescriptor() const
    //noexcept override { 		static const ObjectModelTableEntry table[] = { }; 		static const
    //uint8_t descriptor[] = { 0 }; 		static const ObjectModelClassDescriptor cls{ table,
    //descriptor, nullptr }; 		return &cls;
    //	}
    //
    ...
    //	                  uint8_t /*tableNumber*/,
    //	                  const char *_ecv_array /*filter*/) const THROWS(GCodeException)
    //override
    //	{
    //		buf->cat("{}");
    //	}
    ...
    //	const ObjectModelArrayTableEntry *_ecv_null GetObjectModelArrayEntry(unsigned int)
    //const noexcept override {
    // return nullptr; } 	ReadWriteLock *_ecv_null GetObjectLock(unsigned int) const
    // noexcept override { return nullptr; } 	size_t GetMaxElementsToReturn(const
    // ObjectModelArrayTableEntry*, const ObjectExplorationContext&) const noexcept
    // override { return 0; }
...
private:
    ...
    bool IsLoggingActive() const noexcept
    {
