#!/usr/bin/env python3
# Bridge simulavr serial to a PTY and optionally mirror bytes to UDP for WS broadcast.
# Based on Klipper's scripts/avrsim.py by Kevin O'Connor (GPLv3). This variant adds
# a lightweight UDP mirror to feed hp-sim5's klipper_handler in UDP injection mode.

import sys, optparse, time, os, pty, fcntl, termios, errno, socket
import pysimulavr
...
class TerminalIO:
    def __init__(self, udp_host=None, udp_port=None):
        self.fd = -1
        self.sock = None
        self.addr = None
    ...
    def write(self, data):
        # Bytes from AVR TX go to PTY master and optionally UDP
        os.write(self.fd, data)

    def read(self):
        try:
            data = os.read(self.fd, 64)
            # Mirror host->MCU bytes (Klipper output) via UDP
            if self.sock and self.addr and data:
                try:
...
def main():
    ...
    # New: UDP mirror options
    opts.add_option("--udp-host", type="string", dest="udp_host", default=None,
                    help="Send TX bytes to this UDP host (with --udp-port)")
    opts.add_option("--udp-port", type="int", dest="udp_port", default=None,
                    help="Send TX bytes to this UDP port (with --udp-host)")
    ...
    # Setup terminal (with optional UDP mirror)
    io = TerminalIO(options.udp_host, options.udp_port)
    ...
    if options.trace:
        msg += "Trace file: %s\n" % (options.tracefile,)
    if options.udp_host and options.udp_port:
        msg += "UDP mirror: %s:%d\n" % (options.udp_host, options.udp_port)
