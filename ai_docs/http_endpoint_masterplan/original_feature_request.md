Feature Request: Http endpoint for RrfCanPlayer.

In the hp-sim app I've implemented a class called RrfCanPlayer (see the file integrations/rrf/rrfCanPlayer.js). It simulates the real ReprapFirmware by reading pre-computed movement commands, stored in .csv or .can files, and sending them to the physics engine with the right timing.
...
Now we want RrfCanPlayer or a helper class to do something more dynamic. We want it to replicate the behavior of real ReprapFirmware's http endpoint.
...
I believe RrfCanPlayer should interact with the RepRapFirmware Object Model, but I'm not sure about the details how and why. This is what the introduction to the RepRapFirmware Object Model says in the wiki:
```
...
It's really the gcode endpoint that's the most important. Whatever's sent there should be sent to rrf_simulator which should generate a .csv file or stream or just return value that contains enough info for RrfCanPlayer to \"do the right thing\". We want to support as many gcodes as possible, but start with G1 and M569.4.
