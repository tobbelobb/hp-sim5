...
We opted to decouple mass properties from UsdPhysicsRigidBodyAPI](https://openusd.org/release/api/class_usd_physics_rigid_body_a_p_i.html "Applies physics body attributes to any UsdGeomXformable prim and marks that prim to be driven by a si...") and place them in a separate [UsdPhysicsMassAPI](https://openusd.org/release/api/class_usd_physics_mass_a_p_i.html "Defines explicit mass properties (mass, density, inertia etc.)."). UsdPhysicsMassAPI is not required in cases where the mass properties of an object can be derived from collision geometry (discussed further down in this document) and the UsdPhysicsMaterialAPI. Most commonly, UsdPhysicsMassAPI is applied in addition to UsdPhysicsRigidBodyAPI.
