...
- Cross build targets ship with Eclipse-managed makefiles invoking `arm-none-eabi-g++` with `-mcpu=cortex-m4`, `-mthumb`, floating-point hard ABI, and size-optimised `-Os`.
- Duet2 RTOS configuration exports include search paths for:
  - Core HAL trees (`CoreN2G/src`, `CoreN2G/src/SAM4S_4E_E70/**`, CMSIS 5.4.0 headers, ASF drivers)
  - Firmware sources (`ReprapFirmware/src`, `ReprapFirmware/src/Hardware/SAM4E`, `ReprapFirmware/src/Networking/**`, `ReprapFirmware/src/DuetNG`)
  - Third-party dependencies (FreeRTOS portable layer, WiFi socket server headers, CANlib, RRFLibraries)
- Preprocessor defines used in the firmware build: `__SAM4E8E__`, `RTOS`, `DUET_NG`, `MQTTC_PAL_FILE="Networking/MQTT/mqtt_pal.h"`, `_XOPEN_SOURCE`, and a `noexcept=` compatibility define for the C compiler configuration.
...
- First dry-run compile of `ReprapFirmware/src/Platform/Platform.cpp` exposed the MCU-facing dependencies we must neutralise: `Pins.h` expects board macros (`__SAM4E8E__`, `DUET_NG`), CoreN2G headers like `Interrupts.h`, `AnalogIn/Out.h`, `UniqueIdBase.h`, and SAM-specific device files (`Hardware/SAM4E/Devices.h` pulls in `AsyncSerial` etc.).
- Introduced initial host stubs for `CoreTypes.h`, `Interrupts.h`, `{AnalogIn,AnalogOut}.h`, `UniqueIdBase.h`, cache control, async serial/USB (`AsyncSerial.h`, `SerialCDC.h`, `Wire.h`), and provided runtime stand-ins via `host/src/devices_stub.cpp`, plus extra include dirs for `Hardware/SAM4E` and `DuetNG` during exploratory compiles.
