...
- With these shims in place `g++ … Platform.cpp` now succeeds under the host configuration (warnings only). Next blockers: the transitive includes `Move.h`, `FansManager.h`, etc. will drag in large portions of the runtime, so we should decide whether to stub the dependent subsystems (GCodes, Heat, Tools) before adding `Platform.cpp` to the Makefile target list.
